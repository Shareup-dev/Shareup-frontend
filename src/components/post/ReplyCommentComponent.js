...
export default function ReplyCommentComponent({comment,setRefresh,setReplyCommentFlag,editReply,checkEditReply }) {
    const { user } = useContext(UserContext)
    const ref = useRef(null);
    const [replyContent, setReplyContent] = useState(editReply?editReply.content:"");
  ...
  const handleReplyComment = async(commentId) => {
    if (replyContent === "") {
      return null;
        ...
        const reply = await { content: replyContent }
        console.log(typeof(comment.content),'gggg')
        if(editReply){
          PostService.editReplyForComment(editReply.id,reply).then(res=>{
            checkEditReply(false)
            console.log(res.status)
            setRefresh(res.data)
            setReplyContent("")
          })
        }else{
          PostService.replyComment(user.id, comment.id, reply).then(res => {
            console.log(res.status)
      ...
      <>
        ...
        <li className="post-comment reply-comment">
            ...
            <div className="post-comt-box">
                <Form>
                <textarea rows={2} placeholder={"Reply to "+comment.user.firstName+' '+comment.user.lastName} name="comment" value={replyContent} ref={ref} onKeyPress={(e) => e.key === 'Enter' && handleReplyComment()} onChange={handleReplyContent} />
