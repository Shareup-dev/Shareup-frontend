          ...
          <div className='msgHead'>
            <a href={`/profile/${user.email}`} title={`${user.email}`}>
              <img
                ...
                src={fileStorage.baseUrl + user.profilePicturePath}
                alt=''
              />
              <span
                style={{ paddingLeft: '10px', fontWeight: 'bold', fontSize: '15px' }}
              >{`${user.firstName} ${user.lastName}`}</span>
              <span style={{ paddingLeft: '20px', fontWeight: '900' }}>
                <img src='/assets/images/msgarrw.svg' />
              </span>
            </a>
          </div>
        ...
        <div className='right-message '>
          <div style={{ height: '10%' }}>
            <div className='msgTop'>
              {userR ? (
                <a href={`/profile/${userR.email}`} title={`${userR.email}`}>
                  <img
                    className='msgprof'
                    style={{ objectFit: 'cover' }}
                    src={fileStorage.baseUrl + userR.profilePicturePath}
                    alt=''
                  />
                  <span
                    style={{ paddingLeft: '10px', fontWeight: 'bold', fontSize: '15px' }}
                  >{`${userR.firstName} ${userR.lastName}`}</span>
                </a>
              ) : (
                <a href='#' title='user'>
                  <img
                    ...
                    alt=''
                  />
                  <span style={{ paddingLeft: '10px', fontWeight: 'bold', fontSize: '15px' }}>User</span>
                </a>
              )}
              <div className='msgic'>
                ...
                <div style={{ paddingRight: '10px', display: 'inline' }}>
                  <img src='/assets/images/videoicon.svg' />
                </div>
              </div>
            </div>
          </div>
          <div className='AreaMsg'></div>
          <div className='txtAreaMsg'>
            <input className='chat-text' placeholder='Type message here' name='s'></input>
            <div className='add-microphicon'>
              {' '}
              <img width='20' src='/assets/images/picmsg.svg' />
            </div>
            <div className='add-stckricon'>
              {' '}
              <img width='20' src='/assets/images/stckrmsg.svg' />
            </div>
            <div className='add-picicon'>
              {' '}
